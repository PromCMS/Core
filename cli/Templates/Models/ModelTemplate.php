...
class ModelTemplate extends \PromCMS\Cli\Templates\Models\Abstract\ModelTemplate
{
  ...
  private function updateNamespace()
  {
    ...
    /** @var array<Stmt\Use_> */
    $useStmts = array_filter(
      $namespace->stmts,
      fn($stmt) => $stmt instanceof Stmt\Use_
    );
    $otherStmts = array_filter(
      $namespace->stmts,
      fn($stmt) => $stmt instanceof Stmt\Use_ === false
    );
  ...
  private function updateClass(): void
  {
    ...
    foreach ($class->attrGroups as $attGroup) {
      foreach ($attGroup->attrs as $attr) {
        if (
          $attr->name instanceof Node\Name &&
          $attr->name->name === 'ORM\EntityListeners'
        ) {
          $attributes[] = $attr;
        }
      }
    ...
    if ($isOutputLocalized) {
        ...
        [
          ...
          new Node\Arg(
            ...
            value: new Node\Expr\Array_([
              new Node\Expr\ArrayItem(new Node\Scalar\String_('locale')),
              new Node\Expr\ArrayItem(new Node\Scalar\String_('object_id')),
            ])
          ),
        ]
      );
      ...
      $uniqueConstraintColumns = array_filter(
        $this->entity->getLocalizedColumns(),
        fn(Column|RelationshipColumn $column) => is_string($column->unique)
      );
    } else {
    ...
    foreach ($uniqueConstraintColumns as $uniqueColumn) {
      $uniqueColumnGroups[$uniqueColumn->unique] = array_merge(
        $uniqueColumnGroups[$uniqueColumn->unique] ?? [],
        [$uniqueColumn->name]
      );
    }
      ...
      $attributes[] = new Node\Attribute(
        ...
        [
          ...
          new Node\Arg(
            ...
            value: new Node\Expr\Array_(
              array_map(
                fn($columnName) => new Node\Expr\ArrayItem(
    ...
    $attributes[] = new Node\Attribute(
      new Node\Name('ORM\HasLifecycleCallbacks')
    );

    $class->attrGroups = [new Node\AttributeGroup($attributes)];
  }
