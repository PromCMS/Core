...
class Mailer extends PHPMailer
{
  ...
  public function setup(
    Uri $uri,
    string|null $sendFrom = null,
    string|null $sendFromName = null
  ) {
    ...
    if (!empty(($userInfo = $uri->getUserInfo()))) {
      [$username, $password] = explode(':', urldecode($userInfo));
    ...
    if (!empty(($fromEmailAddress = $sendFrom ?? $this->Username))) {
      $this->setFrom($fromEmailAddress, $sendFromName ?? 'PROM Mailer');
    }
  ...
  public function send(): bool
  {
    if (!$this->isSetuped) {
      $this->ErrorInfo =
        'Mailer is not setuped - setup mailer with setup() method on mailer class';
      return false;
    }
